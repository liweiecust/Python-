...
@login
def getOrganizationChildsPage(orgCode,headers=None):
    # 组织 orgCode
...
@login    
def checkOrgRole(*args):
    # 检查组织有管理员，一般用户角色
    # 组织orgSysNo
    url="https://ibuild.yzw.cn/api/role/organizationRoleList/%s" % args
...
@login
def checkOrganizationMenu(roleSysNo,headers=None): 
...
if __name__=="__main__":
    ...
    # headers=getCookie()
    #获取二级单位列表
    org_tuple=getOrganizationChildsPage(fangyibanOrgCode)
    ...
    for item in orgCodeArray2:
        org_tuple=getOrganizationChildsPage(item)
        for value in org_tuple[0]:
